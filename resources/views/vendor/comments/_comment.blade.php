...
<div class="comment" id="comment-{{ $comment->getKey() }}">
    <div class="media">
        <img src="https://www.gravatar.com/avatar/{{ md5($comment->commenter->email ?? $comment->guest_email) }}.jpg?s=64" alt="{{ $comment->commenter->name ?? $comment->guest_name }} Avatar" class="align-self-start mr-3 rounded-circle">
        <div class="media-body">
            <div class="user-name">{{ $comment->commenter->name ?? $comment->guest_name }}</div>
            <div class="comment-time">{{ $comment->created_at->diffForHumans() }}</div>
            @can('reply-to-comment', $comment)
                <a href="#" class="reply-link" data-toggle="modal" data-target="#reply-modal-{{ $comment->getKey() }}">REPLY</a>
            @endcan
        </div>
    </div>
    ...
    @can('edit-comment', $comment)
        <div class="modal fade" id="comment-modal-{{ $comment->getKey() }}" tabindex="-1" role="dialog" aria-hidden="true" style="display: none;">
    ...
    @can('reply-to-comment', $comment)
        <div class="modal fade" id="reply-modal-{{ $comment->getKey() }}" tabindex="-1" role="dialog" aria-hidden="true" style="display: none;">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
